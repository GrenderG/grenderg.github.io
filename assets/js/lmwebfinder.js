...
function LMWebFinder() {
    /*
     * CONFIGURARTION
     */

    this.primaryColor = "#FFF9C4"; //color for all matches
    this.secondaryColor = "#FFE0B2"; //color for focused matched
    this.findId = "findIn"; //The text you want to search in must be inside an element with this id
    ...
    this.pressKey = function (textToFind, e) {
        var evt = e ? e : event;
        var key = window.Event ? evt.which : evt.keyCode; //return key code from pressed key

        if (key === 13) { //"enter" key
            this.search(textToFind);
        }
    };

    this.search = function (textToFind) {
        this.reset(); //return to original text without colors
        ...
        var space = /^\s$/;
        var searhIn = document.getElementById(this.findId).getElementsByTagName('*');
        var matches = 0;
        ...
        document.getElementById('findIn').innerHTML = document.getElementById(this.findId).innerHTML.split('}}').join('</span>').split('{{').join('<span class="goFind" style="background-color: ' + this.primaryColor + '">'); //replace {{ and }} with span tag with a background color to mark matches
    ...
    this.reset = function () {
        document.getElementById(this.findId).innerHTML = original; //return the text to original version
    };

    this.clear = function (searchInput) {
        searchInput.value = ""; //clears search input
        this.reset(); //return the text to original verion
    };
}